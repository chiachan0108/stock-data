...
def check_refined_fundamentals(sid, token):
    ...
    if isinstance(df, pd.DataFrame) and 'revenue' in df.columns and len(df) >= 24:
        ...
        if cond1 and cond2 and cond3 and ytd_yoy > 0: 
            return sid, True, mom, yoy, ytd_yoy
...
def run_full_pipeline():
    ...
    if os.path.exists(price_cache_file) and not FORCE_RELOAD:
        print(f"⚡ 發現今日 ({today_str}) 的股價快取檔案，直接載入！")
        ...
        for i in range(0, len(all_tickers), batch_size):
            ...
            try:
                ...
                time.sleep(2.5) 
            except Exception as e: print(f"⚠️ 批次下載失敗: {e}")
    ...
    for sid, full_ticker in ticker_map_all.items():
        ...
        try:
            ...
            p, ma60, ma240 = float(df['Close'].iloc[-1]), float(df['Close'].tail(60).mean()), float(df['Close'].tail(240).mean())
            
            # 策略 1 門檻：1,000,000 股 (1,000 張)
            if vol_20_avg >= 1000000 and p > ma240 and ma60 > ma240:
                tech_candidates_1.append(sid)
                metrics_1[sid] = {"現價": p, "季乖離": round(((p-ma60)/ma60)*100, 2), "年乖離": round(((p-ma240)/ma240)*100, 2)}
        except: continue
    ...
    with ThreadPoolExecutor(max_workers=5) as ex:
        ...
        for f in as_completed(futures):
            ...
            if ok and sid in metrics_1:
                chip_net_20 = get_institutional_net_buy(sid, FINMIND_TOKEN_1, 20)
                row = pool_all[pool_all['stock_id'].astype(str) == sid].iloc[0]
                final_list_1.append({
                    "股價代號": sid, "公司名稱": row['stock_name'], "產業別": row['industry_category'],
                    "現價": round(metrics_1[sid]["現價"], 2), "季乖離": metrics_1[sid]["季乖離"], "年乖離": metrics_1[sid]["年乖離"],
                    "月營收MoM(%)": mom, "月營收YoY(%)": yoy, "今年以來累積營收YoY(%)": ytd_yoy,
                    "近20日法人買賣超(張數)": chip_net_20
                })
    
    if final_list_1:
        df_quantum = pd.DataFrame(final_list_1)
        ordered_cols = ["股價代號", "公司名稱", "產業別", "現價", "季乖離", "年乖離", "月營收MoM(%)", "月營收YoY(%)", "今年以來累積營收YoY(%)", "近20日法人買賣超(張數)"]
        df_quantum = df_quantum[ordered_cols].sort_values(by="今年以來累積營收YoY(%)", ascending=False)
    else:
        df_quantum = pd.DataFrame(columns=["股價代號", "公司名稱", "產業別", "現價", "季乖離", "年乖離", "月營收MoM(%)", "月營收YoY(%)", "今年以來累積營收YoY(%)", "近20日法人買賣超(張數)"])
    ...
    for sid, full_ticker in ticker_map_all.items():
        ...
        try:
            ...
            # 💡 [關鍵修正] 策略 2 門檻提升至 1,000 張
            vol_20_lots = float(df['Volume'].tail(20).mean()) / 1000
            if vol_20_lots < 1000: continue
    ...
    for item in tech_candidates_2:
        ...
        if net_buy > 0:
            row = pool_all[pool_all['stock_id'].astype(str) == sid].iloc[0]
            item.update({"公司名稱": row['stock_name'], "產業別": row['industry_category'], "近20日法人買賣超(張)": net_buy})
