...
st.markdown("""
    ...
    .sub-title {
        font-family: 'Noto Sans TC', sans-serif; font-weight: 300; font-size: 1.5rem;
        color: #94a3b8; letter-spacing: 4px; margin-top: 5px; display: block;
    }
    ...
    .status-pill {
        ...
        font-size: 0.85rem; color: #00f2ff; margin-bottom: 30px; margin-top: 20px;
        box-shadow: 0 4px 15px rgba(0, 242, 255, 0.05);
    }
    .pulse-dot {
        height: 8px; width: 8px; background-color: #00f2ff; border-radius: 50%;
        display: inline-block; margin-right: 10px; box-shadow: 0 0 12px #00f2ff;
    ...
    .config-container {
        ...
        border-radius: 20px; padding: 30px; margin-bottom: 40px; box-shadow: inset 0 1px 1px rgba(255,255,255,0.05);
    }
    .section-header {
        font-family: 'JetBrains Mono', monospace; font-size: 0.9rem; letter-spacing: 2px;
    ...
    .logic-item { 
        ...
        border-top: 3px solid rgba(0, 242, 255, 0.25); /* 頂部邊框高光 */
    ...
    .logic-item:hover { 
        ...
        box-shadow: 0 20px 40px -20px rgba(0, 242, 255, 0.15);
    }
    
    /* 💡 圖示與編號對齊排版 */
    .logic-header { display: flex; align-items: center; margin-bottom: 12px; gap: 8px; }
    .logic-icon { font-size: 1.1rem; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.5)); }
    .logic-index { font-family: 'JetBrains Mono', monospace; font-size: 0.85rem; color: #00f2ff; opacity: 0.9; letter-spacing: 0.5px; margin: 0; }
    
    .logic-subtitle { font-size: 1.2rem; font-weight: 700; color: #ffffff; margin-bottom: 10px; letter-spacing: 0.5px; }
    ...
    .disclaimer-box {
        ...
        border-radius: 8px; margin: 40px 0; font-size: 0.9rem; color: #64748b; line-height: 1.6;
    }

    /* 💡 Streamlit 按鈕深度客製化 (漸層光影) */
    .stButton > button {
        background: linear-gradient(135deg, #00f2ff 0%, #0072ff 100%);
        color: white; border: none; border-radius: 12px; font-weight: 600; letter-spacing: 1px;
        padding: 12px 24px; box-shadow: 0 4px 15px rgba(0, 114, 255, 0.3); transition: all 0.3s ease;
    }
    .stButton > button:hover {
        transform: translateY(-2px); box-shadow: 0 8px 25px rgba(0, 114, 255, 0.5);
    }
    .stButton > button:active { transform: translateY(1px); }
    
    .stSelectbox [data-baseweb="select"] {
        background-color: rgba(15, 23, 42, 0.5); border-radius: 12px;
        border: 1px solid rgba(0, 242, 255, 0.2); transition: all 0.3s;
    }
    .stSelectbox [data-baseweb="select"]:hover { border-color: #00f2ff; box-shadow: 0 0 10px rgba(0, 242, 255, 0.1); }
    </style>
...
if not st.session_state['scan_completed']:
    ...
    if "A." in strategy_choice:
        ...
        <div class="logic-grid">
            ...
            <div class="logic-item"><div class="logic-header"><span class="logic-icon">👑</span><span class="logic-index">02 / FUNDAMENTAL</span></div><div class="logic-subtitle">基本面護城河</div><div class="logic-desc">營收創 5 年新高，且近1季與 <span class="highlight">今年累計營收</span> 皆為正成長。</div></div>
            ...
            <div class="logic-item"><div class="logic-header"><span class="logic-icon">🏦</span><span class="logic-index">04 / SMART MONEY</span></div><div class="logic-subtitle">法人雙重認同</div><div class="logic-desc">確保近 <span class="highlight">20 日三大法人</span> 淨買超且營收為正成長。</div></div>
        </div>"""
    ...
    with btn_col:
        ...
        if st.button(f"🚀 啟動【{display_name}】AI量化篩選系統", type="secondary", use_container_width=True): # 改用 secondary 因為已用 CSS 覆蓋所有按鈕樣式
    ...
    if not df.empty:
        ...
        st.download_button("📥 下載完整數據報告 (Excel)", output.getvalue(), file_name=f"Quant_Report_{data_date}.xlsx")
    else:
        st.info("💡 目前暫無符合標的。")
